...
export default function ResearchReport({
  ...
  const handleQA = async () => {
    setCn(true);
    try {
      const res = await axios.post("/api/namespace", {
        research_id: id,
        report: report,
      });
      if (res.data.success) {
  ...
  return (
    <Card className={`${preview && "inset-0 fixed z-50"} rounded-none `}>
      {" "}
          ...
          <div className="h-[500px] border rounded-xl flex items-center justify-center">
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground max-w-md">
                The report is being generated in real-time. You can follow the
                progress in the agent stream and logs above.
              </p>
            </div>
          </div>
        ) : (
          <div className="w-full overflow-x-auto">
            <div className="max-h-[90vh] w-full rounded-xl border sm:p-6 p-2 overflow-auto">
              <article
                id="report"
                className="prose prose-neutral dark:prose-invert max-w-full w-full break-words"
              >
